...
const getGroup = (groupId, groups) => {
    return head(
        groups.filter(
            (subGroup) => isObject(subGroup) && subGroup.id === groupId
        )
    );
};
...
const getLayersId = (groupId, allLayers) => {
    return allLayers
        .filter((layer) => (layer.group || "Default") === groupId)
        .map((layer) => layer.id)
        .reverse();
};
/**
 ...
 */
export const getWpsUrl = (l) =>
    (l && l.wpsUrl) || (l.search && l.search.url) || l.url;
                    ...
                    .reduce((layers, node) => {
                        if (isObject(node)) {
                            return layers.concat(
                                initialReorderLayers([node], allLayers)
                            );
                        }
...
const getElevationDimension = (dimensions = []) => {
    return dimensions.reduce((previous, dim) => {
        return dim.name.toLowerCase() === "elevation" ||
            dim.name.toLowerCase() === "depth"
            ? assign(
                  {
                  ...
                  {
                      name:
                          dim.name.toLowerCase() === "elevation"
                              ? dim.name
                              : "DIM_" + dim.name,
                  }
              )
...
const isSupportedLayerFunc = (layer, maptype) => {
    ...
    if (layer.type === "mapquest" || layer.type === "bing") {
        return (
            Layers.isSupported(layer.type) &&
            layer.apiKey &&
            layer.apiKey !== "__API_KEY_MAPQUEST__" &&
            !layer.invalid
        );
    }
...
export const flattenArrayOfObjects = (groupDetails) => {
    let result = [];
    groupDetails &&
        castArray(groupDetails).forEach((a) => {
            result.push(a);
            if (
                a.nodes &&
 ...
 */
export const getSourceId = (layer = {}) =>
    layer.capabilitiesURL || head(castArray(layer.url));
/**
...
export const extractTileMatrixFromSources = (sources, layer) => {
    ...
    const matrixIds =
        (layer.matrixIds &&
            layer.matrixIds.reduce((a, mI) => {
                const ids =
                    (sources[sourceId] &&
                        sources[sourceId].tileMatrixSet &&
                        sources[sourceId].tileMatrixSet[mI] &&
                        sources[sourceId].tileMatrixSet[mI].TileMatrix.map(
                            (i) => ({
    ...
    const tileMatrixSet =
        (layer.tileMatrixSet &&
            layer.matrixIds
                .map((mI) => sources[sourceId].tileMatrixSet[mI])
                .filter((v) => v)) ||
        null;
 ...
 */
export const extractTileMatrixSetFromLayers = (
    sourcesFromLayers,
    sources = {}
) => {
    return (
        (sourcesFromLayers &&
            Object.keys(sourcesFromLayers).reduce((src, url) => {
                const matrixIds = sourcesFromLayers[url].reduce((a, b) => {
                    return assign(a, {
                        [b.id || b.name]: {
                            srs: [...Object.keys(b.matrixIds)],
                            matrixIds: assign({}, b.matrixIds),
                        },
                    });
                }, {});

                const newMatrixSet = sourcesFromLayers[url].reduce((nMS, l) => {
                    const matrixSetObject = l.tileMatrixSet.reduce(
                        (i, tM) =>
                            assign({}, i, {
                                [tM["ows:Identifier"]]: assign({}, tM),
                            }),
                        {}
                    );

                    const matrixFilteredByLayers = Object.keys(
                        matrixSetObject
                    ).reduce((mFBL, key) => {
                        const layers = Object.keys(matrixIds)
                            .filter((layerId) =>
                                head(
                                    matrixIds[layerId].srs.filter(
                                        (mId) => mId === key
                                    )
                                )
                            )
                            .map(
                                (layerId) => matrixIds[layerId].matrixIds[key]
                            );

                        const TileMatrix =
                            layers[0] &&
                            matrixSetObject[key].TileMatrix.map((m, idx) =>
                                layers[0][idx] && layers[0][idx].ranges
                                    ? assign({}, m, {
                                          ranges: layers[0][idx].ranges,
                                      })
                                    : assign({}, m)
                            );
                        ...
                        return !head(layers)
                            ? assign({}, mFBL)
                            : assign({}, mFBL, {
                                  [key]: assign({}, matrixSetObject[key], {
                                      TileMatrix,
                                  }),
                              });
                    }, {});
                ...
                return assign({}, src, {
                    [url]: assign({}, sources[url] || {}, {
                        tileMatrixSet: assign(
                            {},
                            (src[url] && src[url].tileMatrixSet) || {},
                            newMatrixSet
                        ),
                    }),
                });
...
export const extractSourcesFromLayers = (layers) => {
    ...
    const groupByUrl = layers
        .filter((l) => l.tileMatrixSet)
        .reduce((a, l) => {
            const sourceId = getSourceId(l);
            return a[sourceId]
                ? assign({}, a, { [sourceId]: [...a[sourceId], l] })
                : assign({}, a, { [sourceId]: [l] });
        }, {});
...
export const extractDataFromSources = (mapState) => {
    ...
    const sources =
        (mapState.mapInitialConfig &&
            mapState.mapInitialConfig.sources &&
            assign({}, mapState.mapInitialConfig.sources)) ||
        {};

    return !isEmpty(sources)
        ? mapState.layers.map((l) => {
...
export const getGroupByName = (groupName, groups = []) => {
    ...
    return (
        ...
        groups.reduce(
            (prev, g) =>
                prev ||
                (!!g.nodes && LayersUtils.getGroupByName(groupName, g.nodes)),
            undefined
        )
    );
};
...
export const getLayerId = (layerObj) => {
    return (
        (layerObj && layerObj.id) ||
        `${layerObj.name ? `${layerObj.name}__` : ""}${uuidv1()}`
    );
};
/**
...
export const normalizeLayer = (layer) =>
    layer.id ? layer : { ...layer, id: LayersUtils.getLayerId(layer) };
/**
...
export const normalizeMap = (rawMap = {}) =>
    [
        (map) =>
            (map.layers || []).filter(({ id } = {}) => !id).length > 0
                ? {
                      ...map,
                      layers: (map.layers || []).map((l) =>
                          LayersUtils.normalizeLayer(l)
                      ),
                  }
                : map,
        (map) =>
            map.groups
                ? map
                : { ...map, groups: { id: "Default", expanded: true } },
 ...
 */
export const belongsToGroup = (gid) => (l) =>
    (l.group || "Default") === gid || (l.group || "").indexOf(`${gid}.`) === 0;
export const getLayersByGroup = (configLayers, configGroups) => {
    let i = 0;
    let mapLayers = configLayers.map((layer) =>
        assign({}, layer, { storeIndex: i++ })
    );
    let groupNames = mapLayers
        .reduce((groups, layer) => {
            return groups.indexOf(layer.group || "Default") === -1
                ? groups.concat([layer.group || "Default"])
                : groups;
        }, [])
    ...
    return groupNames.reduce((groups, names) => {
        ...
        name.split(".").reduce((subGroups, groupName, idx, array) => {
            ...
            if (!group) {
                ...
                group.nodes = getLayersId(groupId, mapLayers)
                    .concat(group.nodes)
                    .reduce((arr, cur) => {
                        isObject(cur)
                            ? arr.push({
                                  node: cur,
                                  order: mapLayers.find(
                                      (el) => el.group === cur.id
                                  )?.storeIndex,
                              })
                            : arr.push({
                                  node: cur,
                                  order: mapLayers.find((el) => el.id === cur)
                                      ?.storeIndex,
                              });
                        return arr;
                    }, [])
...
export const getNotEmptyGroup = (group) => {
    const nodes = group.nodes.reduce((gNodes, node) => {
        return node.nodes
            ? gNodes.concat(LayersUtils.getNotEmptyGroup(node))
            : gNodes.concat(node);
    }, []);
...
export const denormalizeGroups = (allLayers, groups) => {
    ...
    let normalizedLayers = allLayers.map((layer) =>
        assign({}, layer, { expanded: layer.expanded || false })
    );
    return {
        flat: normalizedLayers,
        groups: groups.map((group) =>
            getNormalizedGroup(group, normalizedLayers)
        ),
    };
};
...
export const splitMapAndLayers = (mapState) => {
    if (mapState && isArray(mapState.layers)) {
        let groups = LayersUtils.getLayersByGroup(
            mapState.layers,
            mapState.groups
        );
        ...
        if (isArray(mapState.groups)) {
            groups = mapState.groups.reduce((g, group) => {
                ...
                if (group.title) {
                    ...
                    newGroups = LayersUtils.deepChange(
                        newGroups,
                        group.id,
                        groupMetadata
                    );
...
export const geoJSONToLayer = (geoJSON, id) => {
    ...
    if (geoJSON.type === "FeatureCollection") {
        ...
        features = [
            pick(
                { ...geoJSON, id: isNil(geoJSON.id) ? uuidv1() : geoJSON.id },
                ["geometry", "type", "style", "id"]
            ),
        ];
    }
    return {
    ...
    return assign(
        {
            ...
            format: layer.format,
            thumbURL:
                layer.thumbURL && layer.thumbURL.split(":")[0] === "blob"
                    ? undefined
                    : layer.thumbURL,
            group: layer.group,
            search: layer.search,
        ...
        layer.localizedLayerStyles
            ? { localizedLayerStyles: layer.localizedLayerStyles }
            : {},
...
export const getCapabilitiesUrl = (layer) => {
    const matchedGeoServerName = LayersUtils.findGeoServerName({
        url: layer.url,
    });
...
export const invalidateUnsupportedLayer = (layer, maptype) => {
    return isSupportedLayerFunc(layer, maptype)
        ? checkInvalidParam(layer)
        : assign({}, layer, { invalid: true });
};
/**
...
export const getLayerTitleTranslations = (capabilities) => {
    return !!LayerCustomUtils.getLayerTitleTranslations
        ? LayerCustomUtils.getLayerTitleTranslations(capabilities)
        : capabilities.Title;
};
...
export const excludeGoogleBackground = (ll) => {
    const hasVisibleGoogleBackground =
        ll.filter(
            ({ type, group, visibility } = {}) =>
                group === "background" && type === "google" && visibility
        ).length > 0;
    ...
    if (
        hasVisibleGoogleBackground &&
        backgrounds.filter(({ visibility } = {}) => visibility).length === 0
    ) {
        // select the first available
        if (backgrounds.length > 0) {
            const candidate = findIndex(layers, { group: "background" });
            return layers.map((l, i) =>
                i === candidate ? { ...l, visibility: true } : l
            );
        }
...
export const creditsToAttribution = ({ imageUrl, link, title }) => {
    ...
    const html = imageUrl
        ? `<img src="${imageUrl}" ${title ? `title="${title}"` : ``}>`
        : title;
    return link && html
...
export const formatCapabitiliesOptions = function (capabilities) {
    return isObject(capabilities)
        ? {
              ...
              availableStyles:
                  capabilities.style &&
                  (Array.isArray(capabilities.style)
                      ? capabilities.style
                      : [capabilities.style]),
          }
        : {};
};
export const getLayerTitle = ({ title, name }, currentLocale = "default") =>
    title?.[currentLocale] || title?.default || title || name;
...
export const visibleTimelineLayers = (layers, timelineLayers) => {
    return layers.filter((layer) => {
        let timelineLayer = timelineLayers?.find(
